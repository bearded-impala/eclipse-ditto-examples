...
def cleanup_ditto():
    ...
    try:
            ...
            if policy_id:
                ...
                try:
                    ...
                    if response.status_code in (204, 404):
                        ...
                        logger.warning(f"⚠️ Failed to delete policy {policy_id} (HTTP {response.status_code})")
        ...
        for connection_id in known_connection_ids:
            ...
            try:
                ...
                if delete_response.status_code in (204, 404):
                    ...
                    logger.debug(f"Connection {connection_id} not found or already deleted")
            except Exception as e:
        ...
        connection_endpoints = [
            f"{os.getenv('DITTO_API_BASE')}/connections"
        ]
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(connections, list):
                        for connection in connections:
                            ...
                            if connection_id and connection_id not in known_connection_ids:
                                ...
                                for delete_url in delete_urls:
                                    try:
                                        ...
                                        if delete_response.status_code in (204, 404):
                                            ...
                                            logger.debug(f"Failed to delete connection {connection_id} from {delete_url} (HTTP {delete_response.status_code})")
                                    except Exception as e:
                                        logger.debug(f"Error deleting connection {connection_id} from {delete_url}: {e}")
                                ...
                                if not deleted:
                                    try:
                                        ...
                                        if delete_response.status_code in (200, 204, 404):
                                            ...
                                            logger.debug(f"Failed to delete connection {connection_id} via POST command (HTTP {delete_response.status_code})")
                                    except Exception as e:
                                        logger.debug(f"Error deleting connection {connection_id} via POST command: {e}")
                                
                                if not deleted:
                                    logger.warning(f"⚠️ Could not delete connection {connection_id} using any method")
                    break  # If we found a working endpoint, stop trying others
                else:
                    logger.debug(f"Could not list connections from {endpoint} (HTTP {response.status_code})")
            except Exception as e:
                logger.debug(f"Could not list connections from {endpoint}: {e}")
        else:
            logger.debug("Could not list connections from any endpoint - this is normal for some Ditto versions")
