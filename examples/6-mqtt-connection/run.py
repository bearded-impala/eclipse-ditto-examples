...
class MQTTConnectionExample(ExampleRunner):
    ...
    def start_mqtt_broker(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(["uv", "run", "poe", "start-mqtt-broker"], 
    ...
    def create_connection_from_file(self, filename: str, connection_type: str) -> bool:
        ...
        try:
            ...
            if 200 <= response.status_code < 300:
                ...
                self.logger.error(f"❌ Failed to create {connection_type} connection (HTTP {response.status_code})")
                return False
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            operations = [
                ...
                ("Creating Ditto things", lambda: self.create_thing_from_file(self.sensor01_id, "sensor01.json") and self.create_thing_from_file(self.sensor02_id, "sensor02.json")),
                ("Creating MQTT connections", lambda: self.create_connection_from_file("connection_source.json", "MQTT source") and self.create_connection_from_file("connection_target.json", "MQTT target")),
            ]
