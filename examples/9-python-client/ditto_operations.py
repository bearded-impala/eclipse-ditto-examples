...
async def example_usage() -> None:
    ...
    try:
        # Test 1: Search for existing things
        print_section("Test 1: Search Things")
        results = await search_things()

        # Test 2: Get an existing thing
        if results["items"]:
            first_thing = results["items"][0]
            # Handle both PyPI client response format and fallback
            if hasattr(first_thing, "thing_id"):
                thing_id = first_thing.thing_id
            elif isinstance(first_thing, dict) and "thingId" in first_thing:
                thing_id = first_thing["thingId"]
            else:
                thing_id = "org.eclipse.ditto:000001"  # fallback
            print_section("Test 2: Get Existing Thing")
            await get_thing(thing_id)

        # Test 3: Create a new policy
        print_section("Test 3: Create Policy")
        test_policy_id = "org.eclipse.ditto:test-policy"
        await create_policy(test_policy_id, "policy.json")

        # Test 4: Create a new thing
        print_section("Test 4: Create Thing")
        test_thing_id = "org.eclipse.ditto:test-thing"
        await create_thing(test_thing_id, "thing.json")

        # Test 5: Update thing properties
        print_section("Test 5: Update Thing Properties")
        await update_thing_property(test_thing_id, "attributes/manufacturer", "Updated Test Company")
        await update_thing_property(test_thing_id, "features/temperature/properties/value", 25.0)
        await update_thing_property(test_thing_id, "features/humidity/properties/value", 50.0)

        # Test 6: Get the updated thing
        print_section("Test 6: Get Updated Thing")
        await get_thing(test_thing_id)

        # Test 7: Create a connection using regular API
        print_section("Test 7: Create Connection (Regular API)")
        await create_connection("connection.json")

        # Test 8: Create a connection using DevOps piggyback API
        print_section("Test 8: Create Connection (DevOps Piggyback)")
        await create_connection_piggyback("connection_piggyback.json")
