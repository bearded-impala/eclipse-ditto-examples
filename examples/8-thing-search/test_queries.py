...
def execute_query(
    client: httpx.Client, rql: str, ditto_url: str, auth_user: str, auth_pass: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        response = client.get(
            url, params=params, auth=(auth_user, auth_pass), timeout=60.0
        )
...
def get_all_things(
    ...
    try:
        ...
        response = client.get(url, auth=(auth_user, auth_pass), timeout=60.0)
...
def main():
    parser = argparse.ArgumentParser(
        description="Execute RQL queries against Eclipse Ditto"
    )
    parser.add_argument(
        "--queries",
        type=str,
        default="queries/camera.json",
    ...
    try:
        for i, (description, rql) in enumerate(queries, 1):
            ...
            result = execute_query(
                client, rql, args.ditto_url, args.auth_user, args.auth_pass
            )
