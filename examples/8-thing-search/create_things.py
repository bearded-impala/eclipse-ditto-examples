...
def create_policy(client: httpx.Client, ditto_url: str, auth_user: str, auth_pass: str) -> bool:
    ...
    policy_data = {
        "entries": {
            "DEFAULT": {
                "subjects": {
                    "{{ request:subjectId }}": {"type": "generated"}
                },
                "resources": {
    ...
    try:
        ...
        if 200 <= response.status_code < 300:
            ...
            print(f"❌ Failed to create policy {policy_id}. Status: {response.status_code}")
            return False
...
def create_thing(client: httpx.Client, thing_id: str, thing_data: Dict[str, Any], 
                ditto_url: str, auth_user: str, auth_pass: str) -> bool:
...
def main():
    ...
    parser.add_argument("--count", type=int, required=True, 
                       help="Number of things to create")
    parser.add_argument("--schema", type=str, default="schema/camera.json",
    ...
    try:
        ...
        for i in range(args.count):
            ...
            # Create the thing
            if create_thing(client, thing_id, thing_data, args.ditto_url, args.auth_user, args.auth_pass):
                success_count += 1
